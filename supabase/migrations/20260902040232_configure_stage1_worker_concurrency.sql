-- Analytical independence is mandatory; execution concurrency is not an
-- analytical rule. The server supplies this technical setting on each claim.
...
declare
    ...
    may_submit boolean;
begin
    if p_maximum_parallel_cases is null
       or p_maximum_parallel_cases < 1 then
...
comment on function public.claim_available_advanced_preliminary_analysis(
    ...
    'Claims or polls Stage 1 work using server-supplied technical concurrency. Analytical independence remains mandatory and is unrelated to this value.';
