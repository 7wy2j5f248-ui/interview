create function private.extract_complete_stage1_report_array_v2(
    p_source text,
    p_key text
)
returns jsonb
...
begin
    ...
    if key_position = 0 then return '[]'::jsonb; end if;
    array_position := strpos(substr(p_source, key_position), '[')
        + key_position - 1;
    if array_position < key_position then return '[]'::jsonb; end if;
    for i in array_position..length(p_source) loop
        ...
        if in_string then
            ...
            if depth = 0 then
                begin
                    return substr(
                        p_source, array_position, i - array_position + 1
                    )::jsonb;
...
create function private.backfill_stage1_readable_reports_v2(
    p_from integer,
    p_to integer
)
returns integer
