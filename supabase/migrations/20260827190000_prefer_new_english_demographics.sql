...
begin
    ...
    set
        ...
        gender = coalesce(excluded.gender, descriptor.gender),
        age = coalesce(excluded.age, descriptor.age),
        birth_year = coalesce(excluded.birth_year, descriptor.birth_year),
...
where job.archived_at is null
  ...
  and exists (
      ...
      where session.session_id = job.session_id
        ...
        and lower(coalesce(session.language, '')) <> 'en'
  )
  and exists (
      select 1
