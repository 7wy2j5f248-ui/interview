...
add column if not exists analysis_hierarchy_audit jsonb not null default '{}'::jsonb;

comment on column public.qualitative_case_reports.analysis_hierarchy_audit is
    'Researcher-visible platform audit for multi-code theme support, semantic coverage, higher-level abstraction, anti-paraphrase, coherent participant story, project-topic fit, and ungrouped review-needed codes.';

alter table public.automatic_case_analysis_jobs
alter column analysis_version
set default 'case-analysis-v4-theme-hierarchy-audited';
...
declare
    ...
    hierarchy_audit jsonb := coalesce(
        p_payload -> 'analysisHierarchyAudit',
        '{}'::jsonb
    );
    code_entry record;
...
begin
    ...
    if jsonb_typeof(p_payload -> 'codes') <> 'array'
       ...
       or jsonb_typeof(extracted_sources) <> 'object'
       or jsonb_typeof(hierarchy_audit) <> 'object'
       or hierarchy_audit ->> 'complete' <> 'true' then
    ...
    insert into public.qualitative_case_reports (
        ...
        source_completed_at, input_token_count, project_id,
        analysis_framework_id, analysis_hierarchy_audit
    ) values (
        job.session_id,
        job.case_number,
        job.participant_id,
    ...
    loop
        if jsonb_typeof(theme_entry.value -> 'codeNumbers') <> 'array'
           or jsonb_array_length(theme_entry.value -> 'codeNumbers') < 2 then
            raise exception 'Every theme requires at least two supporting codes.';
        end if;
