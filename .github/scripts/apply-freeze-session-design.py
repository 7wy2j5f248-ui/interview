...
replace_once(
    ...
    '''            p_language: requiredIdentifier(language, "Language"),
            p_interview_model: requestedModel,
            p_research_design_id: requiredIdentifier(
                researchDesignId,
                "Research design"
            ),'''
)
...
replace_once(
    ...
    '''        interviewModel: normalizeOpenAIModel(
        ...
        researchDesignId: requiredIdentifier(
            result.selected_research_design_id || researchDesignId,
            "Research design"
        )
    };'''
)
...
replace_once(
    ...
    '''    const activeSessionId = preparedSession.sessionId;
    const sessionDesign = await loadResearchDesignById(
      supabaseClient,
...
for old, new in [
    ("${design.research_goal}", "${sessionDesign.research_goal}"),
    ("${design.ai_role}", "${sessionDesign.ai_role}"),
    ("${design.interview_question_count}", "${sessionDesign.interview_question_count}"),
    ("${design.ending_message}", "${sessionDesign.ending_message}"),
    ("${design.interview_questions}", "${sessionDesign.interview_questions}"),
    ("${design.maximum_interviewer_questions}", "${sessionDesign.maximum_interviewer_questions}")
]:
    text = (ROOT / "api/chat.js").read_text(encoding="utf-8")
    (ROOT / "api/chat.js").write_text(text.replace(old, new), encoding="utf-8")
...
begin
    ...
    if nullif(btrim(p_session_id), '') is null
       ...
       or not exists (
            select 1 from public.research_designs d
            where d.id = p_research_design_id
       )
    ...
    if not found then
        insert into public.interview_sessions (
            session_id, participant_id, language, interview_model,
            research_design_id, completed, completed_at, created_at,
            updated_at, last_activity_at, session_status,
            inactivity_timeout_minutes
        ) values (
            btrim(p_session_id), btrim(p_participant_id), btrim(p_language),
            normalized_model, p_research_design_id, false, null,
            p_request_at, p_request_at, null, 'active', p_timeout_minutes
        );

        return query select btrim(p_session_id), null::text, false, true,
    ...
    calculated_timeout_at := coalesce(
        ...
        existing_session.last_activity_at + make_interval(mins => p_timeout_minutes)
    );

    if existing_session.session_status in ('timed_out', 'abandoned')
       or (existing_session.last_activity_at is not null
           and p_request_at > existing_session.last_activity_at
               + make_interval(mins => p_timeout_minutes)) then
        if not existing_session.completed and existing_session.session_status = 'active' then
            calculated_timeout_at := existing_session.last_activity_at
                + make_interval(mins => p_timeout_minutes);
            update public.interview_sessions
            set session_status = 'timed_out', end_reason = 'inactivity_timeout',
                timed_out_at = calculated_timeout_at, ended_at = calculated_timeout_at
            where session_id = existing_session.session_id;
        end if;

        new_session_id := 'S' || floor(extract(epoch from p_request_at) * 1000)::bigint::text
        ...
        insert into public.interview_sessions (
            ...
            research_design_id, completed, completed_at, created_at,
            updated_at, last_activity_at, session_status,
            continuation_of_session_id, inactivity_timeout_minutes
        ) values (
            new_session_id, btrim(p_participant_id), btrim(p_language),
            normalized_model, p_research_design_id, false, null,
            p_request_at, p_request_at, null, 'active',
            existing_session.session_id, p_timeout_minutes
        );

        return query select new_session_id, existing_session.session_id,
    ...
    set inactivity_timeout_minutes = p_timeout_minutes,
        research_design_id = coalesce(research_design_id, p_research_design_id)
    where session_id = existing_session.session_id;

    return query select existing_session.session_id, null::text, false, false,
...
write(
    "tests/session-design-freeze.test.mjs",
    '''import assert from "node:assert/strict";
import { readFile } from "node:fs/promises";
import test from "node:test";

test("sessions freeze a research-design identity", async () => {
  const [chat, lifecycle, migration] = await Promise.all([
    readFile(new URL("../api/chat.js", import.meta.url), "utf8"),
    readFile(new URL("../server/sessionLifecycle.js", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260719050000_freeze_research_design_per_session.sql", import.meta.url), "utf8")
  ]);

  assert.match(chat, /researchDesignId: design\.id/);
  assert.match(chat, /loadResearchDesignById[\s\S]*preparedSession\.researchDesignId/);
  assert.match(chat, /sessionDesign\.ai_role/);
  assert.match(chat, /sessionDesign\.interview_questions/);
  assert.match(lifecycle, /p_research_design_id/);
  assert.match(lifecycle, /selected_research_design_id/);
  assert.match(migration, /research_design_id uuid references public\.research_designs\(id\)/);
  assert.match(migration, /coalesce\(existing_session\.research_design_id, p_research_design_id\)/);
});
'''
)

print("Applied session research-design freezing.")
