    ...
    async function downloadStage1Report(cohort, button) {
        ...
        try {
            ...
            const version = response.headers.get("X-Stage1-Workbook-Version")
                || "current";
            status.textContent = `One complete Stage 1 Excel workbook report downloaded (${version}). All ${cases} cohort cases are together in that workbook. For this pilot only, worksheet 1 uses demographic columns A:O from the actual saved GPT-5.1 complete-case report. That report contains 272 cohort cases; P0042, P0047, and P0049 have blank demographic fields because they are absent from it. No GPT-5.1 analytical worksheet or analytical process is included. Meaning Units, Codes, Categories, Themes, and Notes & Sources come only from the frozen GPT-5.6 Stage 1 reports. Future Stage 1 workbooks use participant information generated by the selected Stage 1 model. No AI call, validator, reviewer, repair, or retry was used to create this download.`;
        } catch (error) {
            status.textContent = error.message;
            status.className = "error";
        } finally {
