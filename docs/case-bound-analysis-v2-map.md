...
| One selected AI per run | Provider/model labels are run-level, but runtime settings and prompt are partly environment-derived | Provider, exact model, reasoning, rules, project context, schema, and literal request are frozen per attempt; Stage 1 freezes its researcher-selected output allowance, while Stage 2 imposes no application-level output-token ceiling |
