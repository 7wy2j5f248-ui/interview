...
test("researcher dashboard uses four separate case, keyword, code, and theme forms", async () => {
    ...
    assert.match(html, /independent, case-grounded\s*Meaning Unit identification/);
    assert.match(html, /historical\s*preliminary-analysis provenance/);
    assert.match(html, /Active researcher-controlled sequence/);
    assert.match(html, /Meaning Units →\s*Codes → Categories → Themes/);
    assert.match(html, /Forms 1–4 are preserved below only as historical/);
    assert.match(html, /cannot control or block the\s*staged workflow/);
...
test("separate Cases and Keywords forms load every case and stored unit", async () => {
    ...
    assert.match(html, /researcher-automatic-analysis\.js\?version=20260901-stage1-launch-v2/);
    assert.match(html, /researcher-advanced-preliminary\.js\?version=20260901-stage1-launch-v2/);
