...
test("researcher dashboard uses four separate case, keyword, code, and theme forms", async () => {
    ...
    assert.match(html, /independent,\s*case-grounded\s*Meaning Unit identification/);
    assert.match(html, /Historical preliminary reports and jobs remain preserved/);
    assert.match(html, /Active researcher-controlled sequence/);
    assert.match(html, /Meaning Units →\s*Codes → Categories → Themes/);
    assert.match(html, /Historical preliminary reports and jobs remain preserved/);
    assert.match(html, /retired Forms 1–4[\s\S]*not part of this active dashboard/);
    assert.match(html, /MU = Meaning Unit · CO = Code ·\s*CA = Category · TH = Theme/);
...
test("separate Cases and Keywords forms load every case and stored unit", async () => {
    ...
    assert.match(html, /researcher-automatic-analysis\.js\?version=20260901-stage1-only-v3/);
    assert.match(html, /researcher-advanced-preliminary\.js\?version=20260901-stage1-only-v3/);
    assert.match(script, /function revealUnlockedWorkspace/);
    assert.match(script, /Stage 1 remains separate above/);
    assert.match(script, /if \(token\(\)\) \{[\s\S]*revealUnlockedWorkspace\(\)/);
    assert.match(script, /const stagedOnly = workspace\.dataset\.stagedOnly === "true"/);
    assert.match(script, /if \(!stagedOnly\) \{[\s\S]*await load\(\)/);
