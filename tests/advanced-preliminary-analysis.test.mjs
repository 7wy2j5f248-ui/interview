...
test("researcher UI locks later stages and exposes model, audit, evidence, and export provenance", async () => {
    ...
    assert.match(html, /Historical forms[\s\S]*cannot hide or disable[\s\S]*Stage 1 launch control/);
    ...
    assert.match(html, /Historical preliminary outputs — read-only audit reference/);
    assert.match(html, /retired all-in-one[\s\S]*provenance and[\s\S]*comparison only/);
